...
Je crée une petite TODO pour suivre la réponse rapide (vérifier l'URL d'administration).

Création de 1 tâche à faire

- **Réponse** : L'URL d'administration est https://mercan.pythonanywhere.com/admin/  
- **Login** : https://mercan.pythonanywhere.com/admin/login  
- **Remarque** : le blueprint `admin` est enregistré sous le préfixe `/admin` dans app.py; si la page est inaccessible, vérifie que l'utilisateur admin existe et que les variables d'environnement (`ADMIN_USERNAME`, `ADMIN_PASSWORD_HASH`) sont correctement configurées.